...
int main() {
   ...
   while (fgets(buffer, sizeof(buffer), stdin)) {
      ...
      int nameIndex = 0;
      while(nameIndex < strlen(buffer)){
         if(buffer[nameIndex] == NAME_DELIM){
            break;
         }
         nameIndex++;
      }

      char* name = new char[nameIndex];
      char* phone = new char[strlen(buffer) - 1 - nameIndex];
      for(int a = 0; a < nameIndex; a++){
         name[a] = buffer[a];
      }
      int phoneIndex = 0;
      for(int a = nameIndex + 1; a < strlen(buffer); ){
         phone[phoneIndex++] = buffer[a++];
      }

      char* areaCode;
      char* prefix;
      char* lineNumber;
      readField(phone, 0, AREA_CODE_SIZE, areaCode);
      readField(phone, strlen(areaCode) + 1, strlen(areaCode) + 1 + PREFIX_SIZE, prefix);
      readField(phone, strlen(areaCode) + 1 + strlen(prefix) + 1, strlen(areaCode) + 1 + strlen(prefix) + 1 + LINE_NO_SIZE, lineNumber);
